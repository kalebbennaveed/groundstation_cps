...
bool dasc_msgs__msg__di_trajectory__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("dasc_msgs.msg._di_trajectory.DITrajectory", full_classname_dest, 41) == 0);
  }
  dasc_msgs__msg__DITrajectory * ros_message = _ros_message;
  {  // header
  ...
  {  // twists
    ...
    geometry_msgs__msg__Twist * dest = ros_message->twists.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
  ...
  {  // accelerations
    ...
    if (!geometry_msgs__msg__Accel__Sequence__init(&(ros_message->accelerations), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create geometry_msgs__msg__Accel__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    geometry_msgs__msg__Accel * dest = ros_message->accelerations.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!geometry_msgs__msg__accel__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of DITrajectory */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("dasc_msgs.msg._di_trajectory");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "DITrajectory");
  ...
  {  // accelerations
    ...
    geometry_msgs__msg__Accel * item;
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->accelerations.data[i]);
      PyObject * pyitem = geometry_msgs__msg__accel__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
