...
class TrajectorySetpoint(metaclass=Metaclass_TrajectorySetpoint):
    ...
    def __init__(self, **kwargs):
        ...
        self.raw_mode = kwargs.get('raw_mode', bool())
        if 'cmd' not in kwargs:
            self.cmd = numpy.zeros(4, dtype=numpy.float32)
        else:
            self.cmd = numpy.array(kwargs.get('cmd'), dtype=numpy.float32)
            assert self.cmd.shape == (4, )
    ...
    def __eq__(self, other):
        ...
        if self.yawspeed != other.yawspeed:
            return False
        if self.raw_mode != other.raw_mode:
            return False
    ...
    def raw_mode(self, value):
        if __debug__:
            assert \
                ...
                "The 'raw_mode' field must be of type 'bool'"
    ...
    def cmd(self, value):
        if isinstance(value, numpy.ndarray):
            assert value.dtype == numpy.float32, \
                "The 'cmd' numpy.ndarray() must have the dtype of 'numpy.float32'"
            assert value.size == 4, \
                "The 'cmd' numpy.ndarray() must have a size of 4"
            self._cmd = value
            return
        if __debug__:
            from collections.abc import Sequence
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 True), \
                "The 'cmd' field must be a set or sequence with length 4 and each value of type 'float'"
        self._cmd = numpy.array(value, dtype=numpy.float32)
